...
main()
{
  sp_cx_mat H,jp,jp0,dJz;//,ho1,ho2;//, hold, n0, I, n,tn,dJx2,tm;
  //cx_mat Da,Da0;
  ...
  H.set_size(int(Nmax/2)*(nmax+1),int(Nmax/2)*(nmax+1));//here we are setting up the matrix for a^dagger a
  for(i=0;i<Nmax/2;i++){
   for(j=0;j<nmax+1;j++){
     H(i*int(Nmax/2)+j,i*int(Nmax/2)+j)=omega*j;
   }
  }
